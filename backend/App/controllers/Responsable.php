...
use \Core\Controller;
use \App\models\Responsable AS ResponsableDao;
...
class Responsable extends Controller{
    ...
    function __construct(){
        ...
        //if(Controller::getPermisosUsuario($this->__usuario, "seccion_responsables", 1) ==0)
        ...
        $(document).ready(function(){
          ...
          var oTable = $('#muestra-cupones').DataTable({
            ...
            $("#export_pdf").click(function(){
              $('#all').attr('action', '/Responsable/generarPDF/');
              $('#all').attr('target', '_blank');
              $("#all").submit();
            });

            $("#export_excel").click(function(){
              $('#all').attr('action', '/Responsable/generarExcel/');
              $('#all').attr('target', '_blank');
              $("#all").submit();
            });
            ...
            $("#delete").click(function(){
              ...
              if(seleccionados>0){
                alertify.confirm('¿Segúro que desea eliminar lo seleccionado?', function(response){
                  if(response){
                    $('#all').attr('target', '');
                    $('#all').attr('action', '/Responsable/delete');
                    $("#all").submit();
...
html;
      $responsables = ResponsableDao::getAll();
      //$usuario = $this->__usuario;
      //$editarHidden = (Controller::getPermisosUsuario($usuario, "seccion_responsables", 5)==1)? "" : "style=\"display:none;\"";
      //$eliminarHidden = (Controller::getPermisosUsuario($usuario, "seccion_responsables", 6)==1)? "" : "style=\"display:none;\"";
      $tabla= '';
      $status= '';
...
html;
        ...
        $tabla.=<<<html
                <tr>
                <td><input type="checkbox" name="borrar[]" value="{$value['catalogo_responsable_id']}"/></td>
                ...
                <td class="center" >
                    <a href="/Responsable/edit/{$value['catalogo_responsable_id']}" {$editarHidden} type="submit" name="id" class="btn btn-outline-primary"><span class="fa fa-pencil-square-o"></span> </a>
                    <a href="/Responsable/show/{$value['catalogo_responsable_id']}" type="submit" name="id_responsable" class="btn btn-outline-success"><span class="fa fa-eye" ></span> </a>
                </td>
                </tr>
html;
      }

      // $pdfHidden = (Controller::getPermisosUsuario($usuario, "seccion_responsables", 2)==1)?  "" : "style=\"display:none;\"";
      // $excelHidden = (Controller::getPermisosUsuario($usuario, "seccion_responsables", 3)==1)? "" : "style=\"display:none;\"";
      // $agregarHidden = (Controller::getPermisosUsuario($usuario, "seccion_responsables", 4)==1)? "" : "style=\"display:none;\"";
          ...
          $.validator.addMethod("verificarRFC",
            function(value, element) {
              ...
              $.ajax({
                type:"POST",
                async: false,
                url: "/Responsable/validarRFC", // script to validate in server side
                data: {
                    nombre: function() {
                      return $("#rfc").val();
                    }},
          ...
          $.validator.addMethod("verificarRFC",
            function(value, element) {
              ...
              $.ajax({
                type:"POST",
                async: false,
                url: "/Responsable/validarOtroRFC", // script to validate in server side
                data: {
                    nombre: function() {
                      return $("#nombre").val();
                    },
                    id: function(){
                      return $("#catalogo_responsable_id").val();
                    }
                },
                success: function(data) {
                    console.log("success::: " + data);
...
html;
      $responsable = ResponsableDao::getById($id);

      $status = "";
      foreach (ResponsableDao::getStatus() as $key => $value) {
        $selected = ($responsable['status']==$value['catalogo_status_id'])? 'selected' : '';
        $status .=<<<html
...
html;
      ...
      View::set('responsable',$responsable);
      View::set('header',$this->_contenedor->header(''));
      View::set('footer',$this->_contenedor->footer($extraFooter));
      View::render("responsable_edit");
    }
...
html;
    ...
    public function delete(){
      ...
      foreach ($id as $key => $value) {
        $id = ResponsableDao::delete($value);
        if($id['seccion'] == 2){
      ...
      $this->alertas("Eliminacion de Responsables", $array, "/Responsable/");
    }

    public function responsableAdd(){
      $responsable = new \stdClass();

      $nombre = MasterDom::getDataAll('nombre');
      $nombre = MasterDom::procesoAcentosNormal($nombre);
      $responsable->_nombre = $nombre;

      $rest = MasterDom::getDataAll('catalogo_responsable_id')+2;
      $responsable->_id = $rest;

      $apellido_paterno = MasterDom::getDataAll('apellido_paterno');
      $apellido_paterno = MasterDom::procesoAcentosNormal($apellido_paterno);
      $responsable->_apellido_paterno = $apellido_paterno;

      $apellido_materno = MasterDom::getDataAll('apellido_materno');
      $apellido_materno = MasterDom::procesoAcentosNormal($apellido_materno);
      $responsable->_apellido_materno = $apellido_materno;

      $razon_social = MasterDom::getDataAll('razon_social');
      $razon_social = MasterDom::procesoAcentosNormal($razon_social);
      $responsable->_razon_social = $razon_social;

      $email= MasterDom::getDataAll('email');
      $email= MasterDom::procesoAcentosNormal($email);
      $responsable->_email = $email;

      $telefono= MasterDom::getDataAll('telefono');
      $responsable->_telefono = $telefono;

      $rest = substr($nombre, 0, 2)+substr($apellido_paterno, 0, 2);
      $responsable->_clave = $rest;

      $id = ResponsableDao::insert($responsable);
      if($id >= 0){
        $this->alerta($id,'add');
      }else{
    ...
    public function responsableEdit(){
      $responsable = new \stdClass();
      $responsable->_catalogo_responsable_id = MasterDom::getData('catalogo_responsable_id');
      $id = ResponsableDao::verificarRelacion(MasterDom::getData('catalogo_responsable_id'));
      $nombre = MasterDom::getDataAll('nombre');
      $nombre = MasterDom::procesoAcentosNormal($nombre);
      $responsable->_nombre = $nombre;
      $descripcion = MasterDom::getDataAll('descripcion');
      $descripcion = MasterDom::procesoAcentosNormal($descripcion);
      $responsable->_descripcion = $descripcion;
      $responsable->_status = MasterDom::getData('status');
      ...
      if($id['seccion'] == 2){
        ...
        if($idStatus){
          if(ResponsableDao::update($responsable) > 0)
            $this->alerta($id,'edit');
          else
      ...
      if($id['seccion'] == 1){
        ...
        //$id = ResponsableDao::update($responsable);

        if(MasterDom::getData('status') == 2){
          ResponsableDao::update($responsable);
          $this->alerta(MasterDom::getData('catalogo_responsable_id'),'delete');
        }else{
          if(ResponsableDao::update($responsable) >= 1) $this->alerta($id,'edit');
    ...
    public function validarRFC(){
      $dato = ResponsableDao::getRFC($_POST['rfc']);
      if($dato == 1){
        echo "true";
      }else{
        echo "false";
    ...
    public function validarOtroRFC(){
      $id = ResponsableDao::getIdComparacion($_POST['id'], $_POST['nombre']);
      if($id == 1)
        echo "true";

      if($id == 2){
        $dato = ResponsableDao::getNombreResponsable($_POST['nombre']);
        if($dato == 2){
          echo "true";
        }else{
          echo "false";
...
html;
      ...
      if($ids!=''){
        foreach ($ids as $key => $value) {
          $responsable = ResponsableDao::getByIdReporte($value);
            $tabla.=<<<html
              <tr style="background-color:#B8B8B8;">
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['catalogo_responsable_id']}</td>
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['nombre']}</td>
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['descripcion']}</td>
              <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['status']}</td>
              </tr>
html;
        }
      }else{
        foreach (ResponsableDao::getAll() as $key => $responsable) {
          $tabla.=<<<html
            <tr style="background-color:#B8B8B8;">
            <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['catalogo_responsable_id']}</td>
            <td style="height:auto; width: 200px;background-color:#E4E4E4;">{$responsable['nombre']}</td>
...
html;
      ...
      exit;
      //$ids = MasterDom::getDataAll('borrar');
      //echo shell_exec('php -f /home/granja/backend/public/librerias/mpdf_apis/Api.php Responsable '.json_encode(MasterDom::getDataAll('borrar')));
    }
    ...
    public function generarExcel(){
      ...
      $nombreCampo = array('catalogo_responsable_id','nombre','descripcion','status');

      $objPHPExcel->getActiveSheet()->SetCellValue('A'.$fila, 'Reporte de Responsables');
      ...
      if($ids!=''){
        foreach ($ids as $key => $value) {
          ...
          foreach ($nombreCampo as $key => $campo) {
            $objPHPExcel->getActiveSheet()->SetCellValue($columna[$key].$fila, html_entity_decode($responsable[$campo], ENT_QUOTES, "UTF-8"));
        ...
        foreach (ResponsableDao::getAll() as $key => $value) {
          foreach ($nombreCampo as $key => $campo) {
    ...
    public function alerta($id, $parametro){
      ...
      if($parametro == 'delete'){
        $mensaje = "Se ha eliminado la responsable {$id}, ya que cambiaste el estatus a eliminado";
        $class = "success";
      }
